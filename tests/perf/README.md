...
```bash
node tests/perf/scripts/benchmark-home-ssg.mjs /path/to/base/dist /tmp/home.json 3
node tests/perf/scripts/benchmark-ssg.mjs /path/to/base/dist /tmp/articles.json
node tests/perf/scripts/asset-sizes.mjs /path/to/base/dist /tmp/base-assets.json
node tests/perf/scripts/asset-sizes.mjs dist /tmp/head-assets.json
node tests/perf/scripts/report.mjs /tmp/home.json /tmp/articles.json /tmp/base-assets.json /tmp/head-assets.json /tmp/report.md
```
...
`performance-regression.yml` compares a pull request head with its base commit, or a pushed commit with its first parent. It builds both revisions, runs the performance scenarios, and uploads the raw JSON plus Markdown report. Functional browser route tests run separately through the test workflow. A same-repository pull request receives an updated PR comment; a push to `main` receives a comment on that commit. GitHub restricts write tokens for fork pull requests, so those runs keep the check summary and artifact but skip comments.
