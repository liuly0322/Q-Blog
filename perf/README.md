...
The scripts compare two built static sites served locally with gzip enabled and third-party services mocked.

## Local run

```bash
npm ci --prefix perf
cd perf && npx playwright install chromium
cd ..
pnpm build
```
...
```bash
node perf/scripts/check-ssg.mjs
node perf/scripts/benchmark-home-ssg.mjs /path/to/base/dist /tmp/home.json 3
node perf/scripts/benchmark-ssg.mjs /path/to/base/dist /tmp/articles.json
node perf/scripts/asset-sizes.mjs /path/to/base/dist /tmp/base-assets.json
node perf/scripts/asset-sizes.mjs dist /tmp/head-assets.json
node perf/scripts/report.mjs /tmp/home.json /tmp/articles.json /tmp/base-assets.json /tmp/head-assets.json /tmp/report.md
```

To compare two deployed builds through the same browser/network profile, provide `PERF_BASELINE_URL` and `PERF_HEAD_URL` and run `node perf/scripts/benchmark-live.mjs /tmp/live.json`. This live comparison is separate from CI because CDN caches, edge cold starts, and external network conditions are not stable enough for per-commit regression checks.
...
`performance-regression.yml` compares a pull request head with its base commit, or a pushed commit with its first parent. If one push contains multiple commits, it measures the push tip. It builds both revisions, runs the SSG browser smoke check and the same performance scenarios, and uploads the raw JSON plus Markdown report. A same-repository pull request receives an updated PR comment; a push to `main` receives a comment on that commit. GitHub restricts write tokens for fork pull requests, so those runs keep the check summary and artifact but skip comments.
