...
我们首先根据这个思路说明一些其他语言不可判定，再将这种「规约」看作一个定义在问题集合上的二元关系，形式化，进行进一步讨论。

- $HALT_{\text{TM}}$ 不可判定：假设 $R$ 可以判定 $HALT_{\text{TM}}$，对于 $\langle M, w \rangle$，我们首先调用 $R$ 得到 $M$ 在 $w$ 上是否停机，如果停机可以再模拟 $M$ 在 $w$ 上的行为输出接受或拒绝，否则直接输出拒绝。这样就构造出 $A_\text{TM}$ 的 decider，矛盾。
- $E_{TM}$ 不可判定：假设 $R$ 可以判定 $E_{TM}$，对于 $\langle M, w \rangle$，我们可以构造出一台 $M'$，使得它屏蔽了 $w$ 以外的输出（直接拒绝），只有输入 $w$ 原样转发给 $M$。这样我们可以调用 $R(\langle M' \rangle)$，反转它的输出，得到 $A_\text{TM}$ 的 decider，矛盾。
- $EQ_{TM}$ 不可判定：假设 $R$ 可以判定 $EQ_{TM}$，显然可以用它构造出 $E_{TM}$ 的 decider，这就是把 $E_{TM}$ 归约到 $EQ_{TM}$，同样矛盾。
