...
假设存在一个能接受 $B$ 的 DFA，状态数为 $n$。我们投喂给它一个 $aaa...$ 的前缀。假设投喂完 $k$ 个 $a$ 后，DFA 的状态是 $q_k$。当 $k = n$ 时，连同初始状态，DFA 一共经过了 $n+1$ 个状态，那么必然存在一个 $i<n$，使得 $q_i = q_n$。

这就出现了矛盾：

- 因为 $q_i$ 由投喂 $i$ 个 $a$ 得到，所以再投喂 $i$ 个 $b$ 就应该转移到接受状态；
- 但是因为 $q_i = q_n$，所以再投喂 $i$ 个 $b$ 不足以转移到接受状态。
...
假设 $A$ 是一个正则语言，它的 DFA 状态数为 $n$。则任意 $s \in A$，如果 $|s| \ge n$，则自动机在被投喂前 $n$ 个字符后经过了 $n+1$ 个状态。根据抽屉原理可设 $q_i = q_j (0 \leq i < j \leq n)$。
...
> $|xy| \leq n$ 和 $|y| \ge 1$ 等价于 $0 < i < j \leq n$。

可以试着用 pumping lemma 来说明上一节的 $B$ 不是正则语言。只需要取出一个 $s = a^nb^n$，然后分类讨论 $y$ 无论怎么选取都会导致 $xy^kz \notin B$ 即可。
